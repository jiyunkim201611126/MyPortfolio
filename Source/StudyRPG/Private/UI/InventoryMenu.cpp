...
bool UInventoryMenu::NativeOnDrop(const FGeometry& InGeometry, const FDragDropEvent& InDragDropEvent, UDragDropOperation* InOperation)
{
	...
	if (InventoryOwner && ItemDragDrop->SourceItem)
	{
		...
		{
			InputNumberToDropItemWidget->SetItemToDrop(ItemDragDrop->SourceItem, InventoryOwner);
...
void UInventoryMenu::SetItemToChange(UItemBase* FromItem, IInventoryOwnerInterface* FromOwner, UItemBase* ToItem, IInventoryOwnerInterface* ToOwner)
{
	InputNumberToDropItemWidget->SetItemToChange(FromItem, FromOwner, ToItem, ToOwner);
	InputNumberToDropItemWidget->inputText->SetText(FText::FromString(FString::FromInt(FromItem->Quantity)));
	InputNumberToDropItemWidget->SetVisibility(ESlateVisibility::Visible);
}

void UInventoryMenu::SetItemToSplit(UItemBase* ItemToSplit, IInventoryOwnerInterface* ItemOwner)
{
	InputNumberToDropItemWidget->SetItemToSplit(ItemToSplit, ItemOwner);
