...
// 아이템에 개수 변화 있을 때 불러주는 함수
void UInventoryComponent::RefreshInventoryItemAmount(UItemBase* ItemIn, int32 location, int32 Amount)
{
	if (Amount <= 0)
	{
		UItemBase* NullItem = NewObject<UItemBase>(this, UItemBase::StaticClass());
	...
	{
		InventoryContents[location]->SetQuantity(Amount);
	}
}

// FullStack이 되기 위해 몇 개의 Quantity가 필요한지 반환
...
// 매개변수로 들어온 아이템이 몇 번째 원소인지 반환
int32 UInventoryComponent::GetItemIndex(UItemBase* ItemIn) const
{
	return InventoryContents.IndexOfByKey(ItemIn);
}
...
int32 UInventoryComponent::RemoveAmountOfItem(UItemBase* ItemIn, int32 DesiredAmountToRemove)
{
	...
	RefreshInventoryItemAmount(ItemIn, GetItemIndex(ItemIn), RemainingAmount);
...
int32 UInventoryComponent::ChangeItemWithOtherInventory(UItemBase* ChangeFrom, UItemBase* ChangeTo, bool check, int32 amount)
{
	...
	if (check)
	{
		...
		if (!ChangeTo->isValidItem)
		{
			...
			int32 AbleAddAmount = amount > ChangeFrom->Quantity ? ChangeFrom->Quantity : amount;
	...
	// 2번째 들어옴. 드랍 감지한 슬롯(빈 슬롯임)의 아이템에 amount만큼 넣어야 하는 상황
	else
	{
		// amount만큼의 Quantity를 가진 새로운 아이템을 가리키도록 초기화
		// 새로 만들어주지 않으면 두 슬롯이 같은 아이템을 가리키게 됨
		// AddNewItem은 원하는 슬롯에 넣을 수 없어서 사용 못 함
		UItemBase* NewItem = NewObject<UItemBase>(this, UItemBase::StaticClass());
		NewItem = ChangeTo->CreateItemCopy();
		NewItem->OwningInventory = this;
		NewItem->SetQuantity(amount);

		InventoryContents[GetItemIndex(ChangeFrom)] = NewItem;
...
{
	int fromLocation = GetItemIndex(ChangeFrom);
	int toLocation = GetItemIndex(ChangeTo);
	...
	if (ChangeFrom->ID == ChangeTo->ID && ChangeTo->NumericData.bIsStackable)
	{
		...
		RefreshInventoryItemAmount(ChangeFrom, fromLocation, sumQuantity - itemMaxStackSize);
...
{
	// 현재 인벤토리에 빈 칸이 있고, 요청받은 개수가 아이템의 개수를 넘기지 않는 경우
	if (InventoryNumberOfItems < InventorySlotsCapacity && ItemIn->Quantity > AmountToSplit)
	{
...
void UInventoryComponent::AddNewItem(UItemBase* Item, const int32 AmountToAdd)
{
	...
	for (int counter = 0; counter < InventorySlotsCapacity; counter++)
	{
		if (InventoryContents[counter]->isValidItem == false)
		{
