...
void UInputNumberToDropItemWidget::SetItemToDrop(UItemBase* Item, IInventoryOwnerInterface* Owner, UUserWidget* HaveItemSlot)
{
	if (ItemSlot)
		ItemSlot->SetIsCanClick(true);
	...
	Item1 = Item;
	ItemOwner1 = Owner;
	ItemSlot = Cast<UInventoryItemSlot>(HaveItemSlot);
}

void UInputNumberToDropItemWidget::SetItemToChange(UItemBase* FromItem, IInventoryOwnerInterface* FromOwner, UItemBase* ToItem, IInventoryOwnerInterface* ToOwner, UUserWidget* HaveItemSlot)
{
	if (ItemSlot)
		ItemSlot->SetIsCanClick(true);
...
void UInputNumberToDropItemWidget::SetItemToSplit(UItemBase* ItemToSplit, IInventoryOwnerInterface* ItemOwner, UUserWidget* HaveItemSlot)
{
	if (ItemSlot)
		ItemSlot->SetIsCanClick(true);
