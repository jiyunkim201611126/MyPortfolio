...
void UInventoryItemSlot::SetIsCanClick(bool check)
{
	isCanClick = check;
...
{
	// 클릭할 수 없는 상태거나 빈 슬롯인 경우 바로 리턴
	if (!isCanClick || !ItemReference->isValidItem)
		return FReply::Unhandled();

	FKey WhatIsDown = InMouseEvent.GetEffectingButton();

	// 마우스 우클릭이 감지되면
	if (WhatIsDown == EKeys::RightMouseButton)
	{
	...
	else if (WhatIsDown == EKeys::MiddleMouseButton)
	{
		...
		if (ItemReference->Quantity > 1 && ItemReference->OwningInventory->GetInventoryNumberOfItems() < ItemReference->OwningInventory->GetSlotsCapacity())
		{
			SetIsCanClick(false);
			AMainCharacter* PlayerCharacter = Cast<AMainCharacter>(GetOwningPlayerPawn());
			IInventoryOwnerInterface* ItemOwner = Cast<IInventoryOwnerInterface>(ItemReference->OwningInventory->GetOwner());
			PlayerCharacter->DisplayInputNumberToSplitWidget(ItemReference, ItemOwner, this);
		}

		return FReply::Handled();
	}
	// 마우스 좌클릭이 감지되면
	else if (WhatIsDown == EKeys::LeftMouseButton)
	{
...
// 마우스 더블클릭하면 일반 Down함수가 무시돼서 같은 동작으로 오버라이드
FReply UInventoryItemSlot::NativeOnMouseButtonDoubleClick(const FGeometry& InGeometry, const FPointerEvent& InMouseEvent)
{
	...
	// 마우스 우클릭이 감지되면
	if (WhatIsDown == EKeys::RightMouseButton && ItemReference->isValidItem)
	{
		AMainCharacter* PlayerCharacter = Cast<AMainCharacter>(GetOwningPlayerPawn());
		ItemReference->Use(PlayerCharacter);
...
void UInventoryItemSlot::NativeOnDragDetected(const FGeometry& InGeometry, const FPointerEvent& InMouseEvent, UDragDropOperation*& OutOperation)
{
	...
	if (DragItemVisualClass)
	{
		...
		DragItemOperation->HaveItemSlot = this;
...
bool UInventoryItemSlot::NativeOnDrop(const FGeometry& InGeometry, const FDragDropEvent& InDragDropEvent, UDragDropOperation* InOperation)
{
	...
	if (ItemDragDrop->SourceItem == ItemReference || !isCanClick)
		return true;
	...
	{
		FromInventoryOwner->ChangeItem(ItemDragDrop->SourceItem, ItemReference);
		return true;
	}
	...
	if (FromInventoryOwner && ItemDragDrop->SourceItem)
	{
			...
			&& (!ItemReference->isValidItem || (ItemDragDrop->SourceItem->ID == ItemReference->ID && ItemReference->Quantity < ItemReference->NumericData.MaxStackSize)))
		{
			...
			PlayerCharacter->DisplayInputNumberToDropWidget(ItemDragDrop->SourceItem, FromInventoryOwner, ItemReference, ToInventoryOwner, ItemDragDrop->HaveItemSlot);
